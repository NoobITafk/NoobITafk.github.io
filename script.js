...
/* Hero-only optimization: isolated stylesheet + client-first copy for UA and EN. */
(function optimizeHero(){
  ...
  if(ua){
    ...
    const note=ua.querySelector('.hero-note');if(note)note.remove();
  }
  ...
  if(en){
    ...
    const p=en.querySelector('.hero-text');if(p)p.textContent='Tell me what should happen or what is not working. You do not need to know the technology — I will help define the solution.';
